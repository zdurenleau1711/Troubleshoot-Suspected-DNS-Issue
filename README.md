...
- Internal Server Two: Windows Server 2008 R2 | IP Address: 192.168.12.11



## Steps
1. Log into the first internal machine as admin
...
3. Issue a ping command to the loopback address to verify the TCP socket is on the local machine
...
4. Issue a ping command to the host IP address. This is to test the IP configurations of the local host.
...
5. Now, issue an ipconfig command to look at the TCP/IP configurations on the host machine. We want to find the default gateway address to try to ping.
...
6. Ping the default gateway to see if we can reach it.
...
7. Run the ipconfig command again to take not of the DNS address.
   - C:\Users\Administrator> ipconfig /all
...
9. Run nslookup command to query the DNS
...
10. run nslookup but add the users machine to the end of the command.
...
11. We have concluded that the DNS is not the issue. Based our findings, we could have been provided with the wrong URL. Open a webpage and type the website into the URL.
...
13. After contacting the department, we have been provided with the wrong URL. The URL is `http://www.ips.com`. Type the new URL into the search box to see the results.
